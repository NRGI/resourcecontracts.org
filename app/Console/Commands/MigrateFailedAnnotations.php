...
{
    protected $name = 'nrgi:migrateAnntation';
    ...
    {
        $json = json_decode(file_get_contents(public_path('annotation-file.json')));
        foreach ($json->document->annotations as $annotation) {
            $title    = $this->getAnnotationTitle($annotation->title);
            $anno = [
            ...
            $page = [
                ...
                'annotation'        => [
                    'shapes' => [
                        [
                            "type"     => "rect",
                            "geometry" => $this->migration->convertPoint($annotation->location->image)
                        ]
                    ]
                ],
                'article_reference' => ''
    ...
    public function getAnnotationTitle($title)
    {
        $title = explode('--',$title);
            if (isset($title[0])) {
                return $title[0];
    ...
    protected function mapCategory ($category)
    {
        $list = [
            ...
            "The Domestic Supply Requirement is supplied from Government or Staatsolie entitlements and where the Domestic Supply Requirement cannot be met through these entitlements, Staatsolie may make written request to Kosmos for crude oil entitlements up to 1 year (...)"     => "Other - general",
            "Language"                                                                                                                                                                                                                                                                   => "language",
            ...
            "Partage de production - Eléments de \"Cost Oil\" (base de calcul, limites sur le recouvrement des coûts, e.g. comme % des revenues ou de la production, crédit d'investissement, etc.)//Production Share - \"Cost Oil\" features  (basis of calculation, limits on c (...)" => 'production-share-profit-oil-features-triggers-for-variations-in-split-irr-factor-production-etc',
