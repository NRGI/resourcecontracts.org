		...
		<div class="panel-body">
			...
			<div class="row filter-option">
				<div class="col-md-4 col-sm-6 col-xs-6">
					{!! Form::select('contract', ['all'=>trans('activitylog.all_contract')] + $contracts, Request::only('contract'),['class' =>'form-control']) !!}
				</div>
				<div class="col-md-4 col-sm-6 col-xs-6">
					{!! Form::select('user', ['all'=>trans('activitylog.all_user')] + $users , Request::only('user') ,	['class' =>'form-control']) !!}
				</div>
				<div class="col-md-4 col-sm-6 col-xs-6">
					{!! Form::select('category', ['all'=>trans('activitylog.all_category')] + $categories , Request::only('category') , ['class' =>'form-control']) !!}
				</div>
				@if(!is_null($countries))
					<div class="col-md-4 col-sm-6 col-xs-6">
						{!! Form::select('country', ['all'=>trans('activitylog.all_country')] + $countries , Request::only('country') , ['class' =>'form-control']) !!}
					</div>
				@endif
				<div class="col-md-4 col-sm-6 col-xs-6">
					{!! Form::select('status', ['all'=>trans('activitylog.all_status')] + $status , Request::only('status') ,['class' =>'form-control']) !!}
				</div>
				<div class="col-md-4 col-sm-6 col-xs-6">
			...
			<table class="table table-striped table-responsive">
				...
				@forelse($activityLogs as $activitylog)
					<tr>
						<td>
							<a href="{{route('contract.show',$activitylog->contract_id)}}">{{ $activitylog->contract->metadata->contract_name?$activitylog->contract->metadata->contract_name:''}}</a>
						</td>
						<td>
