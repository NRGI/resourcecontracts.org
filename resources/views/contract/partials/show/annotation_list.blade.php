@if($status == $contract_processing_completed)
	<div class="annotation-wrap block" id="annotations">
		<h3>@if($annotations->count() > 1) <span
					class="annotation-count">{{$annotations->count()}}</span> @endif @lang('contract.annotations')<span
					class="annotation-help">@lang('annotation.edit')</span></h3>
							...
							<div class="row">
								<div class="col-md-10">
									...
									@if(property_exists($child->annotation, "shapes"))
										...
										<span data-pk="{{$child->id}}" data-name="page_no"
											  data-url="{{route('annotation.update')}}"
											  data-value={{$child->page_no}} data-type="select"
