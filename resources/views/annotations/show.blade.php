...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading"> @lang('contract.annotation_list') <span>{{$contract->metadata->contract_name ? $contract->metadata->contract_name : $contract->metadata->project_title}}</span>   <a class="btn btn-default pull-right" href="{{route('contract.show', $contract->id)}}">Back</a> </div>
                    ...
                    @if(count($contract->annotations) > 0)
                        <div class="annotation-wrap">
                            ...
                            @if ($status === Annotation::DRAFT and $current_user->can('complete-annotation'))
                                ...
                                {!!Form::close()!!}
                            @elseif($status === Annotation::COMPLETED and ($current_user->can('publish-annotation') or $current_user->can('reject-annotation') ))
                                {!!Form::open(['route'=>['contract.annotations.status', $contract->id], 'style'=>"display:inline",'method'=>'post'])!!}
